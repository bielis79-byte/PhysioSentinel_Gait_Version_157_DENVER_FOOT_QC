...
def _master_donor_frame(man,side,segment,unit_scale):
    ...
    if segment=="thigh":
        return hip,_frame_from_axis(hip,knee,lateral),max(float(np.linalg.norm(knee-hip)),1e-6)

    if segment=="shank":
        return knee,_frame_from_axis(knee,ankle,lateral),max(float(np.linalg.norm(ankle-knee)),1e-6)

    # Foot: origin at ankle; longitudinal axis heel -> toe.
    forward=_unit(toe-heel,[0,0,1])
    x=lateral-forward*np.dot(lateral,forward)
    x=_unit(x,[1,0,0])
    y=_unit(np.cross(forward,x),[0,1,0])
    x=_unit(np.cross(y,forward),x)
    B=np.stack([x,y,forward],axis=1)
    # V157 · Denver foot orientation regression fix. Internal validation on the
    # 75-frame legacy NPZ showed that a 180° roll around the longitudinal
    # Heel→Toe axis improves bone→skin fit while preserving heel/toe direction.
    # Apply it ONLY to the Denver donor foot frame (x,y flip; z/forward unchanged).
    # No q(t), SKEL joints, skin vertices, thigh/shank or progression direction change.
    B = B @ np.diag([-1.0,-1.0,1.0])
    Lf=max(float(np.linalg.norm(toe-ankle)),1e-6)
    return ankle,B,Lf
...
def build_denver_bone_master(man,max_faces_each=70):
    ...
    root=Path(man["root"])
    cache=root/f"denver_real_bones_master_v157_footroll_{max_faces_each}.npz"
    meta=root/f"denver_real_bones_master_v157_footroll_{max_faces_each}.json"
    ...
    for t in range(1,len(J)):
        ...
        for ip,name in enumerate(master["part_names"]):
            ...
            if len(idx)==0:
                continue
            try:
                o0,ot,R=_rigid_target_delta(J,nm,t,side,seg,None)
